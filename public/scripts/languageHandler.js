...
import {
  handleTranslateFeatureProducts,
...
const checkForLanguageSelection = () => {
  ...
  modalImgs.forEach((imgContainer) =>
...
export const decideLanguageInsertion = () => {
  ...
  const languageToSetID = userLogged
...
const handleChangeLanguage = async (param) => {
  //param es 1/2
  ...
  switch (bodyName) {
    ...
    case "productDetail": //Aca repinto el detalle
      productsFromDB.length &&
        productDetailExportObj.createProductDetailsSection(productsFromDB[0]);
      productDetailExportObj.paintRelatedProductCards &&
        productsFromDB &&
        productDetailExportObj.paintRelatedProductCards();
      break;
    case "category":
...
const updateLanguage = async (lang) => {
  const previousLanguageIDSelected = userLogged
    ? userLogged.payment_type_id
    : getLocalStorageItem("payment_type_id") || null;
...
export const translateProductCards = (container) => {
  ...
  items.forEach((item) => {
    ...
    productNameElement.textContent = isInSpanish
      ? productInDb.es_name
