    ...
    userVerifyExportObj.pageConstructor = function () {
      ...
      resendCode.addEventListener("click", async (e) => {
        try {
          ...
          let response = await fetch(
            `${window.location.origin}/api/user/send-verification-code?user_id=${userLogged.id}`
          );
          ...
          msg = isInSpanish ? response.msg.es : response.msg.en;
      ...
      verifyButton.addEventListener("click", async (e) => {
        try {
          ...
          let response = await fetch(
            `${window.location.origin}/api/user/check-verification-code`,
            {
              method: "POST",
              headers: {
