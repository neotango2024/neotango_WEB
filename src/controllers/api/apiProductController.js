...
const controller = {
  handleGetAllProducts: async (req, res) => {
    try {
      ...
      let products;
      if (productId) {
        const foundProduct = await findProductsInDb(
          productId,
          null,
          true,
          null,
        ...
        const productsFetched = await findProductsInDb(
          null,
          categoryId,
          true,
          limit,
  ...
  handleUpdateProduct: async (req, res) => {
    ...
    const deleteVariationsPromises = variationsToDelete.map(
      async (variationToDelete) => {
...
export async function findProductsInDb(
  ...
  try {
    ...
    if (typeof id === "string") {
      ...
      await setProductKeysToReturn({product: productToReturn, withImages: withImages, withVariations: true}); //Setea las keys para devolver front
...
export async function setProductKeysToReturn({product, withImages = false, withVariations = false}) {
  try {
    //Le seteo la categoria
    product.category = categories.find((cat) => cat.id == product.category_id);
    if(withVariations){
        product.variations = populateVariations(product.variations)
    };
    product.ars_price = minDecimalPlaces(product.ars_price);
    product.usd_price = minDecimalPlaces(product.usd_price);
