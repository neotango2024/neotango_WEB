...
const controller = {
  ...
  handleCreateVariation: async (req, res) => {
    try {
      ...
      const isSuccessfulInsertingVariation = insertVariationsInDb(
        variations,
        productId
      );
  ...
  handleUpdateVariation: async (req, res) => {
    try {
      ...
      const successfulUpdating = await updateVariationInDb(
        variationId,
        quantity
      );
  ...
  handleDeleteVariation: async (req, res) => {
    try {
      ...
      return res.status(HTTP_STATUS.OK.code).json({
        ok: true,
        msg: `${
          areAllPromisesSuccessfull
            ? "All variations were deleted"
            : "Some variations where deleted"
        }`,
      });
    } catch (error) {
...
async function handleVariationToReturn(variation) {
  try {
    ...
    const [variationTacoSizePopulated] =
      populateVariations(variationToPopulate);
    const { taco, size } = variationTacoSizePopulated;
    let productVariation = getDeepCopy(variationToPopulate[0].product);
...
export const getVariationsToDelete = (
  ...
  dbVariations.forEach((dbVar) => {
    const existsInBody = bodyVariations.some((bodyVar) => {
      const isSameSize =
        normalizeToString(dbVar.size_id) === normalizeToString(bodyVar.size_id);
      const isSameProduct =
        normalizeToString(dbVar.product_id) === normalizeToString(productId);
      const isSameTaco =
        normalizeToString(dbVar.taco_id) === normalizeToString(bodyVar.taco_id);
...
export const getVariationsToAdd = (bodyVariations, dbVariations, productId) => {
  ...
  bodyVariations.forEach((bodyVar, index) => {
    const variationExistsInDB = dbVariations.find((dbVar) => {
      const isSameSize =
        normalizeToString(bodyVar.size_id) === normalizeToString(dbVar.size_id);
      const isSameProduct =
        normalizeToString(productId) === normalizeToString(dbVar.product_id); // Comparando el product_id correctamente
      const isSameTaco =
        normalizeToString(bodyVar.taco_id) === normalizeToString(dbVar.taco_id);
